...
var annotations2 = [];
export default function CustomNodeBasicLinePlot(props) {
  const removeStyle = {
    position: "absolute",
    top: 20,
    right: 20,
  ...
  useEffect(() => {
    console.log("came2", traces);
    if (traces) props.parentCallback(traces);
  }, [traces]);
  ...
  console.log("props.xName", props.xName);

  // const addDuplicates = (k, i, mode) => {
  //   props.y.map((item, index) => {
  //     if (k == 0 && mode == 2) sum = sum + parseInt(item)
  //     if (index == i) sum = sum + parseInt(item)
  //   })
  // }

  // const addDup2 = k => {
  //   for (let i = 0; i < props.y.length; i++) {
  //     sum = sum + parseInt(props.y[k])
  //     break
  //   }
  // }
  // useEffect(() => {
  ...
  //     j = 0,
  //     indices = []
  //   for (i = 0; i < props.x.length && j < props.x.length; j++) {
  //     console.log('tr', i, j + 1)
  //     // addDup2(i)
  //     // if (props.x[i] == props.x[j]) addDuplicates(i, j)

  //     if (j == props.x.length - 1) {
  //       i++, (j = i)
  //     }
  //   }
  //   //  console.log('Sum', sum)
    ...
    <>
      ...
      >
        ...
        <div style={{ marginTop: -10 }}>
          <div style={{ overflow: "hidden", height: 310 }}>
            <Plot
              style={{ marginTop: -75 }}
              data={[
                {
